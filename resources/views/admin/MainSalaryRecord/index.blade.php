      ...
      <div class="card-body" id="ajax_responce_serachDiv" style="padding: 0px 5px">
         ...
         <tbody>
            ...
            <tr>
               ...
               <td> @if($info->is_open==1) مفتوح @elseif ($info->is_open==2)
                    <!--a  id="do_close_month" href="/* route('MainSalaryRecord.open_month_admin',$info->id)*/"  class="btn  btn-danger btn-sm"> فتح الأن  </a-->
                    مغلق و مؤرشف @else  بأنتظار الفتح   @endif
                 @if(!empty($info->currnetYear))
                  @if($info->currnetYear['is_open']==1)
                  @if($info->is_open==0 and $info->counterOpenMonth==0 and $info->counterPreviousMonthWatingOpen==0)
                 <button data-id="{{ $info->id }}" href="{{ route('MainSalaryRecord.do_open_month',$info->id) }}" class="btn load_open_monthModel btn-danger btn-sm"> فتح الأن</button>
                 @endif
            @if($info->is_open==1)
              <a  id="do_close_month" href="{{ route('MainSalaryRecord.do_close_month',$info->id) }}"  class="btn  btn-danger btn-sm">  أغلاق وارشفة </a>
                 @endif
                 @endif
                 @endif</td>
            </tr>
...
<script>
 ...
 $(document).on('click','#do_close_month',function(e){
   var res=confirm("هل انت متاكد من اغلاق وارشفه الشهر المالى - ولن تتمكن  مره اخرى من التعديل على الرواتب فى هذا الشهر ومتاح تسويات فقط")
 if(!res){
   return false;
 }
