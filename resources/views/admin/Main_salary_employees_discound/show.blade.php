...
<a href="{{ route('Main_salary_employees_discound.index') }}">  الخصومات   </a>
@endsection
...
<div class="col-12">
   <div class="card">
      <div class="card-header">
         <h3 class="card-title card_title_center">   بيانات  الخصومات   للرواتب الشهر المالى({{ $finance_cln_periods_data['month']->name }}لسنة {{ $finance_cln_periods_data['FINANCE_YR'] }})
      ...
      <form method="POST" action="{{ route('Main_salary_employees_discound.print_search') }}" target="_blank">
         @csrf
      ...
      <div class="row" style="padding: 5px;">
           ...
           <div class="col-md-3">
               <div class="form-group">
                  ...
                  <select  name="discount__types_search" id="discount__types_search" class="form-control select2">
                  <option  value="all">بحث بنوع الخصومات </option>
...
<div class="modal fade " id="AddModal" >
   <div class="modal-dialog modal-xl">
     <div class="modal-content bg-info">
       <div class="modal-header">
         <h4 class="modal-title">أضافة الخصومات  للموظفين بالشهر المالى </h4>
            ...
            <div class="col-md-3 relatet_employees_add" style="display: none;" >
             ...
             <div class="col-md-3">
               <div class="form-group">
                  ...
                  <select  name="discount__types_Add" id="discount__types_Add" class="form-control">
                   @if (@isset($discount__types) && !@empty($discount__types))
                        @foreach ($discount__types as $info )
                        <option value="{{ $info->id }}"> {{ $info->name }}  </option>
                        @endforeach
              ...
              <div class="col-md-3">
              <div class="form-group">
                  <label>أجمالى قيمة الخصم </label>
                  <input  type="text" name="total_Add" id="total_Add" oninput="this.value=this.value.replace(/[^0-9.]/g,'');" class="form-control" value="" >
               </div>
          </div>  
          ...
          <div class="col-md-3">
              ...
              <div class="col-md-12">
               <div class="form-group text-center">
                  <hr>
                  <button id="do_add_now" style="margin-top:33px; " class="btn btn-sm btn-danger" type="submit" name="submit">أضف الخصومات  </button>
               </div>
            </div> 
       </div>
     </div>  
 ...
 <div class="modal fade " id="EditModal" >
   <div class="modal-dialog modal-xl">
     <div class="modal-content bg-info">
       <div class="modal-header">
         <h4 class="modal-title">تحديث الخصومات  للموظفين بالشهر المالى </h4>
         <button type="button" class="close" data-dismiss="modal" aria-label="Close">
           <span aria-hidden="true">&times;</span></button>
       </div>
...
<script>
   ...
   $(document).ready(function(){
      ...
      $(document).on('click','#do_add_now',function(e){
        ...
        var discount__types_Add=$("#discount__types_Add").val();
        ...
        if(employees_code_Add==""){
         ...
         if(discount__types_Add==""){
         alert("من فضلك اختر نوع الخصومات ");
         $("#discount__types_Add").focus();
         return false;
        }
   

         if(total_Add==""||total_Add==0){
         alert("من فضلك أدخل اجمالى  الخصومات ");
         $("#total_Add").focus();
         return false;
        }
   ...
   jQuery.ajax({
   url:'{{ route('Main_salary_employees_discound.checkExsistsBefore') }}',
   type:'post',
   'dataType':'json',
   cache:false,
   ...
   var res=confirm("يوجد خصم سابق مسجل للموظف  هل تريد الاستمرار");
   if(res==true){
      var flagRes=true;
   }else{
       var flagRes=false;
   ...
   jQuery.ajax({
   url:'{{ route('Main_salary_employees_discound.store') }}',
   type:'post',
   'dataType':'html',
   cache:false,
   data:{"_token":'{{ csrf_token() }}',employees_code_Add:employees_code_Add,the_finance_cln_periods_id:the_finance_cln_periods_id,discount__types_Add:discount__types_Add,total_Add:total_Add,notes_Add:notes_Add,day_price_add:day_price_add},
   success: function(data){
     ajax_search();
    setTimeout(function () {
   ...
   alert("عفوا لقد حدث خطأ ");
    ...
    $(document).on('change','#employees_code_search',function(e){
       ...
       $(document).on('change','#discount__types_search',function(e){
        ajax_search(); 
      }); 
   ...
   var discount__types_search=$("#discount__types_search").val();
   var is_archived_search=$("#is_archived_search").val();
   var the_finance_cln_periods_id=$("#the_finance_cln_periods_id").val();
   jQuery.ajax({
   url:'{{ route('Main_salary_employees_discound.ajax_search') }}',
   type:'post',
   'dataType':'html',
   cache:false,
   data:{"_token":'{{ csrf_token() }}',employees_code_search:employees_code_search,discount__types_search:discount__types_search,is_archived_search:is_archived_search,the_finance_cln_periods_id:the_finance_cln_periods_id},
   success: function(data){
   ...
   alert("عفوا لقد حدث خطأ ");
    ...
    $(document).on('click','.delete_this_row',function(e){
      ...
      jQuery.ajax({
      url:'{{ route('Main_salary_employees_discound.delete_row') }}',
      type:'post',
      'dataType':'json',
      cache:false,
    ...
    var main_salary_employee_id=$(this).data("main_salary_employee_id");
      ...
      jQuery.ajax({
      url:'{{ route('Main_salary_employees_discound.load_edit_row') }}',
      type:'post',
      'dataType':'html',
      cache:false,
      data:{"_token":'{{ csrf_token() }}',id:id,the_finance_cln_periods_id:the_finance_cln_periods_id,main_salary_employee_id:main_salary_employee_id},
    ...
    $(document).on('click','#do_edit_now',function(e){
        ...
        var discount__types_edit=$("#discount__types_edit").val();
        ...
        if(employees_code_edit==""){
         ...
         if(discount__types_edit==""){
         alert("من فضلك اختر نوع الخصومات ");
         $("#discount__types_edit").focus();
         return false;
        }
   ...
   jQuery.ajax({
   url:'{{ route('Main_salary_employees_discound.do_edit_row') }}',
   type:'post',
   'dataType':'html',
   cache:false,
   data:{"_token":'{{ csrf_token() }}',employees_code_edit:employees_code_edit,the_finance_cln_periods_id:the_finance_cln_periods_id,main_salary_employee_id:main_salary_employee_id,id:id,discount__types_edit:discount__types_edit,total_edit:total_edit,notes_edit:notes_edit,day_price_edit:day_price_edit},
   success: function(data){
     ajax_search();
    setTimeout(function () {
    $("#backup_freeze_modal").modal("hide");
