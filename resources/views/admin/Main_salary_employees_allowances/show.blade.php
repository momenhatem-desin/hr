...
<a href="{{ route('Main_salary_employees_allowances.index') }}">  البدلات  </a>
@endsection
...
<div class="col-12">
   <div class="card">
      <div class="card-header">
         <h3 class="card-title card_title_center">   بيانات  البدلات  للرواتب الشهر المالى({{ $finance_cln_periods_data['month']->name }}لسنة {{ $finance_cln_periods_data['FINANCE_YR'] }})
      ...
      <form method="POST" action="{{ route('Main_salary_employees_allowances.print_search') }}" target="_blank">
         @csrf
      ...
      <div class="row" style="padding: 5px;">
           ...
           <div class="col-md-3">
               <div class="form-group">
                  <label> بحث بنوع البدلات </label>
                  <select  name="allowances_types_search" id="allowances_types_search" class="form-control select2">
                  <option  value="all">بحث بنوع البدلات</option>
...
<div class="modal fade " id="AddModal" >
   <div class="modal-dialog modal-xl">
     <div class="modal-content bg-info">
       <div class="modal-header">
         <h4 class="modal-title">أضافة البدلات للموظفين بالشهر المالى </h4>
            ...
            <div class="col-md-3 relatet_employees_add" style="display: none;" >
             ...
             <div class="col-md-3">
               <div class="form-group">
                  ...
                  <select  name="allowances_types_Add" id="allowances_types_Add" class="form-control">
                   @if (@isset($allowances_types) && !@empty($allowances_types))
                        @foreach ($allowances_types as $info )
                        <option value="{{ $info->id }}"> {{ $info->name }}  </option>
                        @endforeach
          ...
          <div class="col-md-3">
              ...
              <div class="col-md-12">
               <div class="form-group text-center">
                  <hr>
                  <button id="do_add_now" style="margin-top:33px; " class="btn btn-sm btn-danger" type="submit" name="submit">أضف البدلات </button>
               </div>
            </div> 
       </div>
     </div>  
 ...
 <div class="modal fade " id="EditModal" >
   <div class="modal-dialog modal-xl">
     <div class="modal-content bg-info">
       <div class="modal-header">
         <h4 class="modal-title">تحديث البدلات للموظفين بالشهر المالى </h4>
         <button type="button" class="close" data-dismiss="modal" aria-label="Close">
           <span aria-hidden="true">&times;</span></button>
       </div>
...
<script>
   ...
   $(document).ready(function(){
      ...
      $(document).on('click','#do_add_now',function(e){
        ...
        var allowances_types_Add=$("#allowances_types_Add").val();
        ...
        if(employees_code_Add==""){
         ...
         if(allowances_types_Add==""){
         alert("من فضلك اختر نوع البدلات");
         $("#allowances_types_Add").focus();
         return false;
        }
   

         if(total_Add==""||total_Add==0){
         alert("من فضلك أدخل اجمالى  البدلات");
         $("#total_Add").focus();
         return false;
        }
   ...
   jQuery.ajax({
   url:'{{ route('Main_salary_employees_allowances.checkExsistsBefore') }}',
   type:'post',
   'dataType':'json',
   cache:false,
   ...
   jQuery.ajax({
   url:'{{ route('Main_salary_employees_allowances.store') }}',
   type:'post',
   'dataType':'html',
   cache:false,
   data:{"_token":'{{ csrf_token() }}',employees_code_Add:employees_code_Add,the_finance_cln_periods_id:the_finance_cln_periods_id,allowances_types_Add:allowances_types_Add,total_Add:total_Add,notes_Add:notes_Add,day_price_add:day_price_add},
   success: function(data){
     ajax_search();
    setTimeout(function () {
   ...
   alert("عفوا لقد حدث خطأ ");
    ...
    $(document).on('change','#employees_code_search',function(e){
       ...
       $(document).on('change','#allowances_types_search',function(e){
        ajax_search(); 
      }); 
   ...
   var allowances_types_search=$("#allowances_types_search").val();
   var is_archived_search=$("#is_archived_search").val();
   var the_finance_cln_periods_id=$("#the_finance_cln_periods_id").val();
   jQuery.ajax({
   url:'{{ route('Main_salary_employees_allowances.ajax_search') }}',
   type:'post',
   'dataType':'html',
   cache:false,
   data:{"_token":'{{ csrf_token() }}',employees_code_search:employees_code_search,allowances_types_search:allowances_types_search,is_archived_search:is_archived_search,the_finance_cln_periods_id:the_finance_cln_periods_id},
   success: function(data){
   ...
   alert("عفوا لقد حدث خطأ ");
    ...
    $(document).on('click','.delete_this_row',function(e){
      ...
      jQuery.ajax({
      url:'{{ route('Main_salary_employees_allowances.delete_row') }}',
      type:'post',
      'dataType':'json',
      cache:false,
    ...
    var main_salary_employee_id=$(this).data("main_salary_employee_id");
      ...
      jQuery.ajax({
      url:'{{ route('Main_salary_employees_allowances.load_edit_row') }}',
      type:'post',
      'dataType':'html',
      cache:false,
      data:{"_token":'{{ csrf_token() }}',id:id,the_finance_cln_periods_id:the_finance_cln_periods_id,main_salary_employee_id:main_salary_employee_id},
    ...
    $(document).on('click','#do_edit_now',function(e){
        ...
        var allowances_types_edit=$("#allowances_types_edit").val();
        ...
        if(employees_code_edit==""){
         ...
         if(allowances_types_edit==""){
         alert("من فضلك اختر نوع البدلات");
         $("#allowances_types_edit").focus();
         return false;
        }
   ...
   jQuery.ajax({
   url:'{{ route('Main_salary_employees_allowances.do_edit_row') }}',
   type:'post',
   'dataType':'html',
   cache:false,
   data:{"_token":'{{ csrf_token() }}',employees_code_edit:employees_code_edit,the_finance_cln_periods_id:the_finance_cln_periods_id,main_salary_employee_id:main_salary_employee_id,id:id,allowances_types_edit:allowances_types_edit,total_edit:total_edit,notes_edit:notes_edit,day_price_edit:day_price_edit},
   success: function(data){
     ajax_search();
    setTimeout(function () {
    $("#backup_freeze_modal").modal("hide");
