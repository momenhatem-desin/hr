   ...
   <div class="col-12">
      <div class="card">
         <div class="card-header">
            ...
            <input type="hidden" id="ajax_search_load_add_roles_actions" value="{{ route('admin.permission_roles.load_add_roles_actions') }}">
         ...
         <div class="card-body">
            ...
            <table id="example2" class="table table-bordered table-hover">
               ...
               <tr>
                  ...
                  <td > 
                     ...
                     لايوجد تحديث
                     @endif
                     @if( auth()->user()->is_master_admin==1 or check_permission_sub_menue_actions(151)) 
            ...
            <div class="card-header">
               <h3 class="card-title card_title_center">القوائم الرئيسية المضافة لصلاحية الدور ( {{ $data['name'] }} )  
                  @if( auth()->user()->is_master_admin==1 or check_permission_sub_menue_actions(154)) 
            ...
            <div id="ajax_responce_serarchDiv">
               ...
               @endphp
                     ...
                     <tr>
                        ...
                        <td>
                           @if( auth()->user()->is_master_admin==1 or check_permission_sub_menue_actions(154)) 
                           <a href="{{ route('admin.permission_roles.delete_permission_main_menues',$info->id) }}" class="btn btn-sm btn-danger are_you_sure">حذف</a>
                           @endif
                        </td>
                     </tr>
                     <tr>
                   ...
                   <tr>
                     ...
                     <td>
                        @if( auth()->user()->is_master_admin==1 or check_permission_sub_menue_actions(154)) 
                        <button data-id="{{ $sub->id }}"  class="btn btn-sm load_add_roles_actions btn-success" >اضافة صلاحيات مباشرة  </button>
                       @endif
                       @if( auth()->user()->is_master_admin==1 or check_permission_sub_menue_actions(154)) 
                        <a href="{{ route('admin.permission_roles.delete_permission_sub_menues',$sub->id) }}" class="btn btn-sm btn-danger are_you_sure">حذف</a>
                        @endif
                     </td>
                  </tr>
                  <tr>
                    <td colspan="3" style="text-align: center">
                     @if (@isset($sub->roles_actions) && !@empty($sub->roles_actions) && count($sub->roles_actions) >0)
                   @foreach ( $sub->roles_actions as $action)
                   @if( auth()->user()->is_master_admin==1 or check_permission_sub_menue_actions(154)) 
                   <a style="    background-color: mediumvioletred;" href="{{ route('admin.permission_roles.delete_permission_sub_menues_actions',$action->id) }}" class="btn btn-sm btn-primary are_you_sure">{{ $action->permission_sub_menues_actions_name }} <i class="fa fa-trash" aria-hidden="true"></i></a>
                   @endif
...
<div class="modal fade  "   id="load_add_roles_actionsModal">
   <div class="modal-dialog modal-xl"  >
      <div class="modal-content bg-info">
         <div class="modal-header">
         ...
         <div class="modal-body"  style="background-color: white !important; color:black;" id="load_add_roles_actionsModalBody">
