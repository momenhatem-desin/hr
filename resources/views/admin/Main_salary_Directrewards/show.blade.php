...
<a href="{{ route('Main_salary_Directrewards.index') }}">  المكافئات المباشره  </a>
@endsection
...
<div class="col-12">
   <div class="card">
      <div class="card-header">
         <h3 class="card-title card_title_center">   بيانات  المكافئات المباشره للموظفين الشهر المالى({{ $finance_cln_periods_data['month']->name }}لسنة {{ $finance_cln_periods_data['FINANCE_YR'] }})
      ...
      <form method="POST" action="{{ route('Main_salary_Directrewards.print_search') }}" target="_blank">
         @csrf
...
<div class="modal fade " id="AddModal" >
   <div class="modal-dialog modal-xl">
     <div class="modal-content bg-info">
       <div class="modal-header">
         <h4 class="modal-title">أضافة المكافئات المباشره للموظفين بالشهر المالى </h4>
 ...
 <div class="modal fade " id="EditModal" >
   <div class="modal-dialog modal-xl">
     <div class="modal-content bg-info">
       <div class="modal-header">
         <h4 class="modal-title">تحديث المكافئات المباشره للموظفين بالشهر المالى </h4>
         <button type="button" class="close" data-dismiss="modal" aria-label="Close">
           <span aria-hidden="true">&times;</span></button>
       </div>
...
<script>
   ...
   jQuery.ajax({
   url:'{{ route('Main_salary_Directrewards.checkExsistsBefore') }}',
   type:'post',
   'dataType':'json',
   cache:false,
   ...
   jQuery.ajax({
   url:'{{ route('Main_salary_Directrewards.store') }}',
   type:'post',
   'dataType':'html',
   cache:false,
   ...
   var the_finance_cln_periods_id=$("#the_finance_cln_periods_id").val();
   jQuery.ajax({
   url:'{{ route('Main_salary_Directrewards.ajax_search') }}',
   type:'post',
   'dataType':'html',
   cache:false,
   ...
   alert("عفوا لقد حدث خطأ ");
    ...
    $(document).on('click','.delete_this_row',function(e){
      ...
      jQuery.ajax({
      url:'{{ route('Main_salary_Directrewards.delete_row') }}',
      type:'post',
      'dataType':'json',
      cache:false,
    ...
    var the_finance_cln_periods_id=$("#the_finance_cln_periods_id").val();
      ...
      jQuery.ajax({
      url:'{{ route('Main_salary_Directrewards.load_edit_row') }}',
      type:'post',
      'dataType':'html',
      cache:false,
   ...
   jQuery.ajax({
   url:'{{ route('Main_salary_Directrewards.do_edit_row') }}',
   type:'post',
   'dataType':'html',
   cache:false,
   data:{"_token":'{{ csrf_token() }}',employees_code_edit:employees_code_edit,the_finance_cln_periods_id:the_finance_cln_periods_id,id:id,additional_types_edit:additional_types_edit,total_edit:total_edit,notes_edit:notes_edit,day_price_edit:day_price_edit},
   success: function(data){
     ajax_search();
    setTimeout(function () {
    $("#backup_freeze_modal").modal("hide");
