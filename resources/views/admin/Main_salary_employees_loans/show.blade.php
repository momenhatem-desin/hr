...
<a href="{{ route('Main_salary_employees_loans.index') }}">  السلف الشهرية  </a>
@endsection
...
<div class="col-12">
   <div class="card">
      <div class="card-header">
         <h3 class="card-title card_title_center">   بيانات  السلف الشهرية  للرواتب الشهر المالى({{ $finance_cln_periods_data['month']->name }}لسنة {{ $finance_cln_periods_data['FINANCE_YR'] }})
      ...
      <form method="POST" action="{{ route('Main_salary_employees_loans.print_search') }}" target="_blank">
         @csrf
...
<div class="modal fade " id="AddModal" >
   <div class="modal-dialog modal-xl">
     <div class="modal-content bg-info">
       <div class="modal-header">
         <h4 class="modal-title">أضافة السلف الشهرية للموظفين بالشهر المالى </h4>
         ...
         <div class="row">
           <div class="col-md-3">
            ...
            <div class="col-md-3 relatet_employees_add" style="display: none;" >
              ...
              <div class="col-md-3">
              <div class="form-group">
                  <label>أجمالى قيمة السلفه الشهرية</label>
          ...
          <div class="col-md-3">
              ...
              <div class="col-md-12">
               <div class="form-group text-center">
                  <hr>
                  <button id="do_add_now" style="margin-top:33px; " class="btn btn-sm btn-danger" type="submit" name="submit">أضف السلف الشهرية </button>
               </div>
            </div> 
       </div>
     </div>  
 ...
 <div class="modal fade " id="EditModal" >
   <div class="modal-dialog modal-xl">
     <div class="modal-content bg-info">
       <div class="modal-header">
         <h4 class="modal-title">تحديث السلف الشهرية للموظفين بالشهر المالى </h4>
         <button type="button" class="close" data-dismiss="modal" aria-label="Close">
           <span aria-hidden="true">&times;</span></button>
       </div>
...
<script>
   ...
   $(document).ready(function(){
      ...
      $(document).on('click','#do_add_now',function(e){
        ...
        if(employees_code_Add==""){
         ...
         alert("من فضلك أدخل اجمالى  السلف الشهرية");
         $("#total_Add").focus();
         return false;
        }
   ...
   jQuery.ajax({
   url:'{{ route('Main_salary_employees_loans.checkExsistsBefore') }}',
   type:'post',
   'dataType':'json',
   cache:false,
   ...
   let res=confirm("يوجد سلف سابق مسجل للموظف  هل تريد الاستمرار");
   if(res==true){
       flagRes=true;
   }else{
        flagRes=false;
   ...
   jQuery.ajax({
   url:'{{ route('Main_salary_employees_loans.store') }}',
   type:'post',
   'dataType':'html',
   cache:false,
   data:{"_token":'{{ csrf_token() }}',employees_code_Add:employees_code_Add,the_finance_cln_periods_id:the_finance_cln_periods_id,total_Add:total_Add,notes_Add:notes_Add,day_price_add:day_price_add},
   success: function(data){
     ajax_search();
    setTimeout(function () {
   ...
   var the_finance_cln_periods_id=$("#the_finance_cln_periods_id").val();
   jQuery.ajax({
   url:'{{ route('Main_salary_employees_loans.ajax_search') }}',
   type:'post',
   'dataType':'html',
   cache:false,
   ...
   alert("عفوا لقد حدث خطأ ");
    ...
    $(document).on('click','.delete_this_row',function(e){
      ...
      jQuery.ajax({
      url:'{{ route('Main_salary_employees_loans.delete_row') }}',
      type:'post',
      'dataType':'json',
      cache:false,
    ...
    var main_salary_employee_id=$(this).data("main_salary_employee_id");
      ...
      jQuery.ajax({
      url:'{{ route('Main_salary_employees_loans.load_edit_row') }}',
      type:'post',
      'dataType':'html',
      cache:false,
      data:{"_token":'{{ csrf_token() }}',id:id,the_finance_cln_periods_id:the_finance_cln_periods_id,main_salary_employee_id:main_salary_employee_id},
    ...
    $(document).on('click','#do_edit_now',function(e){
        ...
        if(employees_code_edit==""){
         ...
         alert("من فضلك أدخل اجمالى  السلفه الشهرية");
         $("#employees_code_edit").focus();
         return false;
        }
   ...
   jQuery.ajax({
   url:'{{ route('Main_salary_employees_loans.do_edit_row') }}',
   type:'post',
   'dataType':'html',
   cache:false,
   data:{"_token":'{{ csrf_token() }}',employees_code_edit:employees_code_edit,the_finance_cln_periods_id:the_finance_cln_periods_id,main_salary_employee_id:main_salary_employee_id,id:id,total_edit:total_edit,notes_edit:notes_edit,day_price_edit:day_price_edit},
   success: function(data){
     ajax_search();
    setTimeout(function () {
    $("#backup_freeze_modal").modal("hide");
