...
<body style="padding-top: 10px;font-family: tahoma;">
   ...
   <table style="width: 60%;float: right;  margin-right: 5px;" dir="rtl">
      ...
      <tr>
         <td style="text-align: center;padding: 5px;font-weight: bold;"> <span style=" display: inline-block;
               ...
               بحث  بالتسويات للموظفين   بالشهر المالى ({{ $finance_cln_periods_data['month']->name }}لسنة {{ $finance_cln_periods_data['FINANCE_YR'] }})
            </span>
         </td>
      </tr>
      <tr>
   ...
   <table dir="rtl" id="example2" class="table table-bordered table-hover" style="width: 99%;margin: 0 auto;">
      <thead style="background-color: yellow">
            ...
            <th style="width: 10%;"> اجمالى الاستحقاقات</th>
            <th style="width: 10%;">اجمالى الاستقطاعات</th>
            <th style="width: 10%;">الصافى</th>
            <th style="width: 10%;">  تاريخ </th>
      ...
      <tbody> @php $i=1; @endphp
         ...
         <tr>
                 ...
                 <td> {{ $i }} </td>
               <td> {{ $info->emp_name }} </td>
                <td> {{ $info->total_for}} </td>
               <td> {{ $info->total_on}} </td>
               <td> {{ $info->final_total}} </td>
                <td>
                     @php
         ...
         <tr style="background-color: yellowgreen">
            <td colspan="2">الاجماليات </td>
             <td> {{ $other['total_for']*1}} </td>
             <td> {{$other['total_on']*1}} </td>
            <td colspan="4"> {{ $other['final_total']*1}} </td>
         </tr>
