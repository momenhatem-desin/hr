...
<a href="{{ route('Main_direct_grants.index') }}">  المنح المباشره  </a>
@endsection
...
<div class="col-12">
   <div class="card">
      <div class="card-header">
         <h3 class="card-title card_title_center">   بيانات  المنح المباشره للموظفين الشهر المالى({{ $finance_cln_periods_data['month']->name }}لسنة {{ $finance_cln_periods_data['FINANCE_YR'] }})
      ...
      <form method="POST" action="{{ route('Main_direct_grants.print_search') }}" target="_blank">
         @csrf
      ...
      <div class="row" style="padding: 5px;">
           ...
           <div class="col-md-3">
               <div class="form-group">
                  ...
                  <select  name="grant_types_search" id="grant_types_search" class="form-control select2">
                  <option  value="all">بحث بنوع المكافئات</option>
...
<div class="modal fade " id="AddModal" >
   <div class="modal-dialog modal-xl">
     <div class="modal-content bg-info">
       <div class="modal-header">
         <h4 class="modal-title">أضافة المنح المباشره للموظفين بالشهر المالى </h4>
            ...
            <div class="col-md-3 relatet_employees_add" style="display: none;" >
             ...
             <div class="col-md-3">
               <div class="form-group">
                  ...
                  <select  name="grant_types_Add" id="grant_types_Add" class="form-control">
                   @if (@isset($grant_types) && !@empty($grant_types))
                        @foreach ($grant_types as $info )
                        <option value="{{ $info->id }}"> {{ $info->name }}  </option>
                        @endforeach
          ...
          <div class="col-md-3">
              ...
              <div class="col-md-12">
               <div class="form-group text-center">
                  <hr>
                  <button id="do_add_now" style="margin-top:33px; " class="btn btn-sm btn-danger" type="submit" name="submit">أضف المكافئات </button>
               </div>
            </div> 
       </div>
     </div>  
 ...
 <div class="modal fade " id="EditModal" >
   <div class="modal-dialog modal-xl">
     <div class="modal-content bg-info">
       <div class="modal-header">
         <h4 class="modal-title">تحديث المنح المباشره للموظفين بالشهر المالى </h4>
         <button type="button" class="close" data-dismiss="modal" aria-label="Close">
           <span aria-hidden="true">&times;</span></button>
       </div>
...
<script>
   ...
   $(document).ready(function(){
      ...
      $(document).on('click','#do_add_now',function(e){
        ...
        var grant_types_Add=$("#grant_types_Add").val();
        ...
        if(employees_code_Add==""){
         ...
         if(grant_types_Add==""){
         alert("من فضلك اختر نوع المكافئات");
         $("#grant_types_Add").focus();
         return false;
        }
   

         if(total_Add==""||total_Add==0){
         alert("من فضلك أدخل اجمالى  المكافئات");
         $("#total_Add").focus();
         return false;
        }
   ...
   jQuery.ajax({
   url:'{{ route('Main_direct_grants.checkExsistsBefore') }}',
   type:'post',
   'dataType':'json',
   cache:false,
   ...
   jQuery.ajax({
   url:'{{ route('Main_direct_grants.store') }}',
   type:'post',
   'dataType':'html',
   cache:false,
   data:{"_token":'{{ csrf_token() }}',employees_code_Add:employees_code_Add,the_finance_cln_periods_id:the_finance_cln_periods_id,grant_types_Add:grant_types_Add,total_Add:total_Add,notes_Add:notes_Add,day_price_add:day_price_add},
   success: function(data){
     ajax_search();
    setTimeout(function () {
   ...
   alert("عفوا لقد حدث خطأ ");
    ...
    $(document).on('change','#employees_code_search',function(e){
       ...
       $(document).on('change','#grant_types_search',function(e){
        ajax_search(); 
      }); 
   ...
   var grant_types_search=$("#grant_types_search").val();
   var is_archived_search=$("#is_archived_search").val();
   var the_finance_cln_periods_id=$("#the_finance_cln_periods_id").val();
   jQuery.ajax({
   url:'{{ route('Main_direct_grants.ajax_search') }}',
   type:'post',
   'dataType':'html',
   cache:false,
   data:{"_token":'{{ csrf_token() }}',employees_code_search:employees_code_search,grant_types_search:grant_types_search,is_archived_search:is_archived_search,the_finance_cln_periods_id:the_finance_cln_periods_id},
   success: function(data){
   ...
   alert("عفوا لقد حدث خطأ ");
    ...
    $(document).on('click','.delete_this_row',function(e){
      ...
      jQuery.ajax({
      url:'{{ route('Main_direct_grants.delete_row') }}',
      type:'post',
      'dataType':'json',
      cache:false,
    ...
    var the_finance_cln_periods_id=$("#the_finance_cln_periods_id").val();
      ...
      jQuery.ajax({
      url:'{{ route('Main_direct_grants.load_edit_row') }}',
      type:'post',
      'dataType':'html',
      cache:false,
    ...
    $(document).on('click','#do_edit_now',function(e){
        ...
        var grant_types_edit=$("#grant_types_edit").val();
   ...
   jQuery.ajax({
   url:'{{ route('Main_direct_grants.do_edit_row') }}',
   type:'post',
   'dataType':'html',
   cache:false,
   data:{"_token":'{{ csrf_token() }}',employees_code_edit:employees_code_edit,the_finance_cln_periods_id:the_finance_cln_periods_id,id:id,grant_types_edit:grant_types_edit,total_edit:total_edit,notes_edit:notes_edit,day_price_edit:day_price_edit},
   success: function(data){
     ajax_search();
    setTimeout(function () {
    $("#backup_freeze_modal").modal("hide");
