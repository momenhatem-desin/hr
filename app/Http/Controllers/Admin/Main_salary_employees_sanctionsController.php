...
class Main_salary_employees_sanctionsController extends Controller
{
     use GeneralTrait;
...
return view('admin.MainSalarySanctions.index', ['data' => $data]);
}
...
if(empty($finance_cln_periods_data)){
  return redirect()->route('MainSalaryRecord.index')->with(['error' => 'عفوا  غير قادر على الوصول الى البيانات المطلوبة ! ']);
  }
  $data = get_cols_where_p(new Main_salary_employees_sanctions(), array("*"), array("com_code" => $com_code,"finance_cln_periods_id"=>$finance_cln_periods_id),'id','DESC',PC);
  if(!empty($data)){
    foreach($data as $info){
  ...
  return view('admin.MainSalarySanctions.show', ['data' => $data,'finance_cln_periods_data'=>$finance_cln_periods_data,'employess'=> $employess,'employess_for_search'=>$employess_for_search]);
}

public function checkExsistsBefore(Request $request){
    if($request->ajax()){
    $com_code = auth('admin')->user()->com_code;
       $checkExsistsBeforedata=get_count_where( new Main_salary_employees_sanctions(),array("com_code"=>$com_code,"finance_cln_periods_id"=>$request->the_finance_cln_periods_id,"employees_code"=>$request->employees_code_Add)); 
...
if(!empty($finance_cln_periods_data) && !empty($Main_salary_employee_data)){
     DB::beginTransaction();
      ...
      $dataToInsert['sanctions_type'] =$request->sanctions_type_Add;
  ...
  $flag=insert(new Main_salary_employees_sanctions(),$dataToInsert);
       if($flag){
...
$data=Main_salary_employees_sanctions::select("*")->where($field1,$operator1,$value1)->where($field2,$operator2,$value2)->where($field3,$operator3,$value3)->where('finance_cln_periods_id','=',$the_finance_cln_periods_id)->where('com_code','=',$com_code)->orderby('id','DESC')->paginate(PC);
if(!empty($data)){
    foreach($data as $info){
      $info->emp_name=get_field_value(new Employee(),"emp_name",array("com_code"=>$com_code,"employees_code"=>$info->employees_code));
    }
  }
return view('admin.MainSalarySanctions.ajax_search',['data'=>$data]);
}
}
...
if ($request->ajax()) {
    ...
    $data_row=get_cols_where_row(new Main_salary_employees_sanctions(),array("id"),array("com_code"=>$com_code,"id"=>$request->id,'is_archived'=>0,'finance_cln_periods_id'=>$request->the_finance_cln_periods_id,'main_salary_employee_id'=>$request->main_salary_employee_id));
    if(!empty($finance_cln_periods_data and !empty($data_row) and !empty($Main_salary_employee_data))){
  $flag= destroy(new  Main_salary_employees_sanctions(), array("com_code"=>$com_code,'is_archived'=>0,'finance_cln_periods_id'=>$request->the_finance_cln_periods_id,'id'=>$request->id));
     if($flag){
...
if ($request->ajax()) {
     $finance_cln_periods_data=get_cols_where_row(new Finance_cln_periods(),array("id"),array("com_code"=>$com_code,'id'=>$request->the_finance_cln_periods_id,'is_open'=>1));
      $Main_salary_employee_data=get_cols_where_row(new Main_salary_employee(),array("*"),array("com_code"=>$com_code,'finance_cln_periods_id'=>$request->the_finance_cln_periods_id,'id'=>$request->main_salary_employee_id,'is_archived'=>0));
    $data_row=get_cols_where_row(new Main_salary_employees_sanctions(),array("*"),array("com_code"=>$com_code,"id"=>$request->id,'is_archived'=>0,'finance_cln_periods_id'=>$request->the_finance_cln_periods_id,'main_salary_employee_id'=>$request->main_salary_employee_id));
  ...
  if(!empty($employess)){
    ...
    return view('admin.MainSalarySanctions.load_edit_row',['finance_cln_periods_data'=>$finance_cln_periods_data,'Main_salary_employee_data'=>$Main_salary_employee_data,'data_row'=>$data_row,'employess'=>$employess]);
}
}
...
if(!empty($finance_cln_periods_data) && !empty($Main_salary_employee_data) and !empty($data_row) ){
     DB::beginTransaction();
      ...
      $dataToUpdate['sanctions_type'] =$request->sanctions_type_edit;
      ...
      $flag=update(new Main_salary_employees_sanctions(),$dataToUpdate,array("com_code"=>$com_code,"id"=>$request->id,'is_archived'=>0,'finance_cln_periods_id'=>$request->the_finance_cln_periods_id,'main_salary_employee_id'=>$request->main_salary_employee_id));
         if($flag){
      $this->Recalculate_main_salary_employee($request->main_salary_employee_id);
       }
      DB::commit();
...
$other['value_sum']=0;
$other['total_sum']=0;
$data=Main_salary_employees_sanctions::select("*")->where($field1,$operator1,$value1)->where($field2,$operator2,$value2)->where($field3,$operator3,$value3)->where('finance_cln_periods_id','=',$the_finance_cln_periods_id)->where('com_code','=',$com_code)->orderby('id','DESC')->get('');
if(!empty($data)){
    foreach($data as $info){
      $info->emp_name=get_field_value(new Employee(),"emp_name",array("com_code"=>$com_code,"employees_code"=>$info->employees_code));
...
return view('admin.MainSalarySanctions.print_search',['data'=>$data,'finance_cln_periods_data'=>$finance_cln_periods_data,'systemData'=>$systemData,'other'=>$other]);
}
