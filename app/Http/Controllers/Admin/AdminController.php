...
if($info->is_master_admin==1){
    $info->permission_roles_name = "ادارة عليا";
}else{
...
if(!empty($request->email)){
    $checkExists_email = Admin::where(['email' => $request->email])->first();
...
$checkExists_username = Admin::where(['username' => $request->username])->first();
...
if(!empty($request->email)){
    $checkExists_email = Admin::where(['email' => $request->email])->where('id','!=',$id)->first();
    if (!empty($checkExists_email)) {
    return redirect()->back()
    ->with(['error' => 'عفوا البريد الالكتروني للمتسخدم    مسجل من قبل'])
    ->withInput();
...
$checkExists_username = Admin::where(['username' => $request->username])->where('id','!=',$id)->first();
if (!empty($checkExists_username)) {
return redirect()->back()
->with(['error' => 'عفوا اسم المستخدم للدخول   مسجل من قبل'])
->withInput();
...
$data = Admin::where($field1, $operator1, $value1)->where($field2, $operator2, $value2)->where('com_code', '=', $com_code)->orderBy('id', 'DESC')->paginate(PC);
if (!empty($data)) {
    foreach ($data as $info) {
    $info->added_by_admin = Admin::where('id', $info->added_by)->value('name');
    if($info->is_master_admin==1){
