...
use App\Models\MainEmployeesDirectRewards;
use App\Models\Admin_panel_setting;
use App\Models\Main_salary_employee;
use App\Traits\GeneralTrait;

class mainEmployeesDirectRewardsController extends Controller
{
...
return view('admin.Main_salary_Directrewards.index', ['data' => $data]);
}
...
if(empty($finance_cln_periods_data)){
  return redirect()->route('Main_salary_Directrewards.index')->with(['error' => 'عفوا  غير قادر على الوصول الى البيانات المطلوبة ! ']);
  }
  $data = get_cols_where_p(new MainEmployeesDirectRewards(), array("*"), array("com_code" => $com_code,"finance_cln_periods_id"=>$finance_cln_periods_id),'id','DESC',PC);
  if(!empty($data)){
    foreach($data as $info){
  ...
  return view('admin.Main_salary_Directrewards.show', ['data' => $data,'finance_cln_periods_data'=>$finance_cln_periods_data,'employess'=> $employess,'employess_for_search'=>$employess_for_search,'additional_types'=>$additional_types]);
}

public function checkExsistsBefore(Request $request){
    if($request->ajax()){
    $com_code = auth('admin')->user()->com_code;
       $checkExsistsBeforedata=get_count_where( new MainEmployeesDirectRewards(),array("com_code"=>$com_code,"finance_cln_periods_id"=>$request->the_finance_cln_periods_id,"employees_code"=>$request->employees_code_Add)); 
...
if(!empty($finance_cln_periods_data)){
     ...
     $flag= insert(new MainEmployeesDirectRewards(),$dataToInsert);
...
$data=MainEmployeesDirectRewards::select("*")->where($field1,$operator1,$value1)->where($field2,$operator2,$value2)->where($field3,$operator3,$value3)->where('finance_cln_periods_id','=',$the_finance_cln_periods_id)->where('com_code','=',$com_code)->orderby('id','DESC')->paginate(PC);
if(!empty($data)){
    foreach($data as $info){
      $info->emp_name=get_field_value(new Employee(),"emp_name",array("com_code"=>$com_code,"employees_code"=>$info->employees_code));
      $info->additional_name=get_field_value(new Addtional_type(),"name",array("com_code"=>$com_code,"id"=>$info->addtion_types_id));
...
return view('admin.Main_salary_Directrewards.ajax_search',['data'=>$data]);
}
}
...
if ($request->ajax()) {
    $finance_cln_periods_data=get_cols_where_row(new Finance_cln_periods(),array("id"),array("com_code"=>$com_code,'id'=>$request->the_finance_cln_periods_id,'is_open'=>1));
    $data_row=get_cols_where_row(new MainEmployeesDirectRewards(),array("id"),array("com_code"=>$com_code,"id"=>$request->id,'is_archived'=>0,'finance_cln_periods_id'=>$request->the_finance_cln_periods_id,'main_salary_employee_id'=>$request->main_salary_employee_id));
    if(!empty($finance_cln_periods_data and !empty($data_row))){
    $flag=destroy(new  MainEmployeesDirectRewards(), array("com_code"=>$com_code,'is_archived'=>0,'finance_cln_periods_id'=>$request->the_finance_cln_periods_id,'id'=>$request->id));
    return json_encode("done");
    }
}
}
...
if ($request->ajax()) {
     $finance_cln_periods_data=get_cols_where_row(new Finance_cln_periods(),array("id"),array("com_code"=>$com_code,'id'=>$request->the_finance_cln_periods_id,'is_open'=>1));
    $data_row=get_cols_where_row(new MainEmployeesDirectRewards(),array("*"),array("com_code"=>$com_code,"id"=>$request->id,'is_archived'=>0,'finance_cln_periods_id'=>$request->the_finance_cln_periods_id));
  ...
  if(!empty($employess)){
    ...
    return view('admin.Main_salary_Directrewards.load_edit_row',['finance_cln_periods_data'=>$finance_cln_periods_data,'data_row'=>$data_row,'employess'=>$employess,'additional_types'=>$additional_types]);
}
}
...
if(!empty($finance_cln_periods_data) && !empty($data_row) ){
     ...
     $flag= update(new MainEmployeesDirectRewards(),$dataToUpdate,array("com_code"=>$com_code,"id"=>$request->id,'is_archived'=>0,'finance_cln_periods_id'=>$request->the_finance_cln_periods_id));
      DB::commit();
      return json_encode("done");
  }
    }
...
$other['value_sum']=0;
$other['total_sum']=0;
$data=MainEmployeesDirectRewards::select("*")->where($field1,$operator1,$value1)->where($field2,$operator2,$value2)->where($field3,$operator3,$value3)->where('finance_cln_periods_id','=',$the_finance_cln_periods_id)->where('com_code','=',$com_code)->orderby('id','DESC')->get('');
if(!empty($data)){
    foreach($data as $info){
      $info->emp_name=get_field_value(new Employee(),"emp_name",array("com_code"=>$com_code,"employees_code"=>$info->employees_code));
      $info->additional_name=get_field_value(new Addtional_type(),"name",array("com_code"=>$com_code,"id"=>$info->addtion_types_id));
...
return view('admin.Main_salary_Directrewards.print_search',['data'=>$data,'finance_cln_periods_data'=>$finance_cln_periods_data,'systemData'=>$systemData,'other'=>$other]);
}
