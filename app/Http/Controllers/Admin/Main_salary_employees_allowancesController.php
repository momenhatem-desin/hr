...
class Main_salary_employees_allowancesController extends Controller
{
             use GeneralTrait;
...
return view('admin.Main_salary_employees_allowances.index', ['data' => $data]);
}
...
if(empty($finance_cln_periods_data)){
  return redirect()->route('Main_salary_employees_allowances.index')->with(['error' => 'عفوا  غير قادر على الوصول الى البيانات المطلوبة ! ']);
  }
  $data = get_cols_where_p(new Main_salary_employees_allowances(), array("*"), array("com_code" => $com_code,"finance_cln_periods_id"=>$finance_cln_periods_id),'id','DESC',PC);
  if(!empty($data)){
    foreach($data as $info){
      $info->emp_name=get_field_value(new Employee(),"emp_name",array("com_code"=>$com_code,"employees_code"=>$info->employees_code));
      $info->allowances_name=get_field_value(new Allowances(),"name",array("com_code"=>$com_code,"id"=>$info->allowances_id));
    }
  }
  ...
  $allowances_types=get_cols_where(new Allowances(),array("id","name"),array("com_code"=>$com_code));
  ...
  return view('admin.Main_salary_employees_allowances.show', ['data' => $data,'finance_cln_periods_data'=>$finance_cln_periods_data,'employess'=> $employess,'employess_for_search'=>$employess_for_search,'allowances_types'=>$allowances_types]);
}

public function checkExsistsBefore(Request $request){
    if($request->ajax()){
    $com_code = auth('admin')->user()->com_code;
       $checkExsistsBeforedata=get_count_where( new Main_salary_employees_allowances(),array("com_code"=>$com_code,"finance_cln_periods_id"=>$request->the_finance_cln_periods_id,"employees_code"=>$request->employees_code_Add)); 
...
if(!empty($finance_cln_periods_data) && !empty($Main_salary_employee_data)){
     DB::beginTransaction();
      ...
      $dataToInsert['allowances_id'] =$request->allowances_types_Add;
     ...
     $flag= insert(new Main_salary_employees_allowances(),$dataToInsert);
        if($flag){
...
$allowances_types = $request->allowances_types_search;
...
$data=Main_salary_employees_allowances::select("*")->where($field1,$operator1,$value1)->where($field2,$operator2,$value2)->where($field3,$operator3,$value3)->where('finance_cln_periods_id','=',$the_finance_cln_periods_id)->where('com_code','=',$com_code)->orderby('id','DESC')->paginate(PC);
if(!empty($data)){
    foreach($data as $info){
      $info->emp_name=get_field_value(new Employee(),"emp_name",array("com_code"=>$com_code,"employees_code"=>$info->employees_code));
      $info->allowances_name=get_field_value(new Allowances(),"name",array("com_code"=>$com_code,"id"=>$info->allowances_id));
...
return view('admin.Main_salary_employees_allowances.ajax_search',['data'=>$data]);
}
}
...
if ($request->ajax()) {
    ...
    $data_row=get_cols_where_row(new Main_salary_employees_allowances(),array("id"),array("com_code"=>$com_code,"id"=>$request->id,'is_archived'=>0,'finance_cln_periods_id'=>$request->the_finance_cln_periods_id,'main_salary_employee_id'=>$request->main_salary_employee_id));
    if(!empty($finance_cln_periods_data and !empty($data_row) and !empty($Main_salary_employee_data))){
   $flag= destroy(new  Main_salary_employees_allowances(), array("com_code"=>$com_code,'is_archived'=>0,'finance_cln_periods_id'=>$request->the_finance_cln_periods_id,'id'=>$request->id));
      if($flag){
...
if ($request->ajax()) {
     $finance_cln_periods_data=get_cols_where_row(new Finance_cln_periods(),array("id"),array("com_code"=>$com_code,'id'=>$request->the_finance_cln_periods_id,'is_open'=>1));
      $Main_salary_employee_data=get_cols_where_row(new Main_salary_employee(),array("*"),array("com_code"=>$com_code,'finance_cln_periods_id'=>$request->the_finance_cln_periods_id,'id'=>$request->main_salary_employee_id,'is_archived'=>0));
    $data_row=get_cols_where_row(new Main_salary_employees_allowances(),array("*"),array("com_code"=>$com_code,"id"=>$request->id,'is_archived'=>0,'finance_cln_periods_id'=>$request->the_finance_cln_periods_id,'main_salary_employee_id'=>$request->main_salary_employee_id));
  ...
  if(!empty($employess)){
    ...
    return view('admin.Main_salary_employees_allowances.load_edit_row',['finance_cln_periods_data'=>$finance_cln_periods_data,'Main_salary_employee_data'=>$Main_salary_employee_data,'data_row'=>$data_row,'employess'=>$employess,'allowances_types'=>$allowances_types]);
}
}
...
public function do_edit_row(Request $request){
    ...
    $com_code = auth('admin')->user()->com_code;
    $finance_cln_periods_data=get_cols_where_row(new Finance_cln_periods(),array("id"),array("com_code"=>$com_code,'id'=>$request->the_finance_cln_periods_id,'is_open'=>1));
    $Main_salary_employee_data=get_cols_where_row(new Main_salary_employee(),array("*"),array("com_code"=>$com_code,'finance_cln_periods_id'=>$request->the_finance_cln_periods_id,'id'=>$request->main_salary_employee_id,'employees_code'=>$request->employees_code_edit,'is_archived'=>0));
     $data_row=get_cols_where_row(new Main_salary_employees_allowances(),array("*"),array("com_code"=>$com_code,"id"=>$request->id,'is_archived'=>0,'finance_cln_periods_id'=>$request->the_finance_cln_periods_id,'main_salary_employee_id'=>$request->main_salary_employee_id));
if(!empty($finance_cln_periods_data) && !empty($Main_salary_employee_data) and !empty($data_row) ){
     DB::beginTransaction();
      ...
      $dataToUpdate['allowances_id'] =$request->allowances_types_edit;
     ...
     $flag= update(new Main_salary_employees_allowances(),$dataToUpdate,array("com_code"=>$com_code,"id"=>$request->id,'is_archived'=>0,'finance_cln_periods_id'=>$request->the_finance_cln_periods_id,'main_salary_employee_id'=>$request->main_salary_employee_id));
       if($flag){
      $this->Recalculate_main_salary_employee($request->main_salary_employee_id);
       }
      DB::commit();
...
$other['value_sum']=0;
$other['total_sum']=0;
$data=Main_salary_employees_allowances::select("*")->where($field1,$operator1,$value1)->where($field2,$operator2,$value2)->where($field3,$operator3,$value3)->where('finance_cln_periods_id','=',$the_finance_cln_periods_id)->where('com_code','=',$com_code)->orderby('id','DESC')->get('');
if(!empty($data)){
    foreach($data as $info){
      $info->emp_name=get_field_value(new Employee(),"emp_name",array("com_code"=>$com_code,"employees_code"=>$info->employees_code));
      $info->allowances_name=get_field_value(new Allowances(),"name",array("com_code"=>$com_code,"id"=>$info->allowances_id));
...
return view('admin.Main_salary_employees_allowances.print_search',['data'=>$data,'finance_cln_periods_data'=>$finance_cln_periods_data,'systemData'=>$systemData,'other'=>$other]);
}
