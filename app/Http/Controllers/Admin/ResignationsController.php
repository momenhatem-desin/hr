...
if(!empty($data)){
    foreach($data as $info){
        $info->counterUsed=get_count_where(new Employee(),array("com_code"=>$com_code,"Resignations_id"=>$info->id) );
    }
}
...
$counterUsed=get_count_where(new Employee(),array("com_code"=>$com_code,"Resignations_id "=>$id) );
if ($counterUsed>0) {
return redirect()->route('Resignations.index')->with(['error' => 'عفوا لايمكن الحذف لانه مستخدم  من قبل فى النظام  ']);
}
DB::beginTransaction();
