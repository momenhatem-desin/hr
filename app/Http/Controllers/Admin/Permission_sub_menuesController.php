...
class Permission_sub_menuesController extends Controller
{
    public function index()
    {
        ...
        $data = Permission_sub_menues::select()->where('com_code', '=', $com_code)->orderby('id', 'ASC')->paginate(PC);
        if (!empty($data)) {
            foreach ($data as $info) {
                ...
                $info->permission_sub_menues_actions = get_cols_where(new Permission_sub_menues_actions(), array("*"), array('com_code' => $com_code, "permission_sub_menues_id" => $info->id), 'id', 'ASC');
    ...
    public function ajax_search(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = Permission_sub_menues::where($field1, $operator1, $value1)->where($field2, $operator2, $value2)->orderBy('id', 'ASC')->paginate(PC);
            if (!empty($data)) {
                foreach ($data as $info) {
                    $info->Permission_main_menues_name = Permission_main_menues::where('id', $info->permission_main_menues_id)->value('name');
                    $info->added_by_admin = Admin::where('id', $info->added_by)->value('name');
    ...
    public function delete($id)
    {
        ...
        try {
            ...
            if (!empty($item_row)) {
                ...
                if ($flag) {
                    destroy(new Permission_sub_menues_actions(), array("com_code" => $com_code, 'permission_sub_menues_id' => $id));
                    return redirect()->back()
