...
use App\Models\Main_direct_grants;
use App\Models\Admin_panel_setting;
use App\Models\Grants;
use App\Models\Main_salary_employee;

class main_direct_grantsController extends Controller
{
...
return view('admin.Main_direct_grants.index', ['data' => $data]);
}
...
if(empty($finance_cln_periods_data)){
  return redirect()->route('Main_direct_grants.index')->with(['error' => 'عفوا  غير قادر على الوصول الى البيانات المطلوبة ! ']);
  }
  $data = get_cols_where_p(new Main_direct_grants(), array("*"), array("com_code" => $com_code,"finance_cln_periods_id"=>$finance_cln_periods_id),'id','DESC',PC);
  if(!empty($data)){
    foreach($data as $info){
      $info->emp_name=get_field_value(new Employee(),"emp_name",array("com_code"=>$com_code,"employees_code"=>$info->employees_code));
      $info->grant_name=get_field_value(new Grants(),"name",array("com_code"=>$com_code,"id"=>$info->grants_id));
    }
  }
  ...
  $grant_types=get_cols_where(new Grants(),array("id","name"),array("com_code"=>$com_code));
  ...
  return view('admin.Main_direct_grants.show', ['data' => $data,'finance_cln_periods_data'=>$finance_cln_periods_data,'employess'=> $employess,'employess_for_search'=>$employess_for_search,'grant_types'=>$grant_types]);
}

public function checkExsistsBefore(Request $request){
    if($request->ajax()){
    $com_code = auth('admin')->user()->com_code;
       $checkExsistsBeforedata=get_count_where( new Main_direct_grants(),array("com_code"=>$com_code,"finance_cln_periods_id"=>$request->the_finance_cln_periods_id,"employees_code"=>$request->employees_code_Add)); 
...
if(!empty($finance_cln_periods_data)){
     DB::beginTransaction();
      ...
      $dataToInsert['grants_id'] =$request->grant_types_Add;
     ...
     $flag= insert(new Main_direct_grants(),$dataToInsert);
...
$grant_types = $request->grant_types_search;
...
$data=Main_direct_grants::select("*")->where($field1,$operator1,$value1)->where($field2,$operator2,$value2)->where($field3,$operator3,$value3)->where('finance_cln_periods_id','=',$the_finance_cln_periods_id)->where('com_code','=',$com_code)->orderby('id','DESC')->paginate(PC);
if(!empty($data)){
    foreach($data as $info){
      $info->emp_name=get_field_value(new Employee(),"emp_name",array("com_code"=>$com_code,"employees_code"=>$info->employees_code));
      $info->grant_name=get_field_value(new Grants(),"name",array("com_code"=>$com_code,"id"=>$info->grants_id));
...
return view('admin.Main_direct_grants.ajax_search',['data'=>$data]);
}
}
...
if ($request->ajax()) {
    $finance_cln_periods_data=get_cols_where_row(new Finance_cln_periods(),array("id"),array("com_code"=>$com_code,'id'=>$request->the_finance_cln_periods_id,'is_open'=>1));
    $data_row=get_cols_where_row(new Main_direct_grants(),array("id"),array("com_code"=>$com_code,"id"=>$request->id,'is_archived'=>0,'finance_cln_periods_id'=>$request->the_finance_cln_periods_id));
    if(!empty($finance_cln_periods_data and !empty($data_row))){
    $flag=destroy(new  Main_direct_grants(), array("com_code"=>$com_code,'is_archived'=>0,'finance_cln_periods_id'=>$request->the_finance_cln_periods_id,'id'=>$request->id));
    return json_encode("done");
    }
}
}
...
if ($request->ajax()) {
     $finance_cln_periods_data=get_cols_where_row(new Finance_cln_periods(),array("id"),array("com_code"=>$com_code,'id'=>$request->the_finance_cln_periods_id,'is_open'=>1));
    $data_row=get_cols_where_row(new Main_direct_grants(),array("*"),array("com_code"=>$com_code,"id"=>$request->id,'is_archived'=>0,'finance_cln_periods_id'=>$request->the_finance_cln_periods_id));
  ...
  if(!empty($employess)){
    ...
    return view('admin.Main_direct_grants.load_edit_row',['finance_cln_periods_data'=>$finance_cln_periods_data,'data_row'=>$data_row,'employess'=>$employess,'grant_types'=>$grant_types]);
}
}
...
if(!empty($finance_cln_periods_data) && !empty($data_row) ){
     DB::beginTransaction();
      ...
      $dataToUpdate['grants_id'] =$request->grant_types_edit;
     ...
     $flag= update(new Main_direct_grants(),$dataToUpdate,array("com_code"=>$com_code,"id"=>$request->id,'is_archived'=>0,'finance_cln_periods_id'=>$request->the_finance_cln_periods_id));
      DB::commit();
      return json_encode("done");
  }
    }
...
$other['value_sum']=0;
$other['total_sum']=0;
$data=Main_direct_grants::select("*")->where($field1,$operator1,$value1)->where($field2,$operator2,$value2)->where($field3,$operator3,$value3)->where('finance_cln_periods_id','=',$the_finance_cln_periods_id)->where('com_code','=',$com_code)->orderby('id','DESC')->get('');
if(!empty($data)){
    foreach($data as $info){
      $info->emp_name=get_field_value(new Employee(),"emp_name",array("com_code"=>$com_code,"employees_code"=>$info->employees_code));
      $info->grant_name=get_field_value(new Grants(),"name",array("com_code"=>$com_code,"id"=>$info->grants_id));
...
return view('admin.Main_direct_grants.print_search',['data'=>$data,'finance_cln_periods_data'=>$finance_cln_periods_data,'systemData'=>$systemData,'other'=>$other]);
}
