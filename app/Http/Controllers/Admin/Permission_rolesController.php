...
$data = Permission_rols::select()->orderby('id', 'ASC')->paginate(PC);
$data=get_cols_where_p(new Permission_rols(),array("*"),array("com_code"=>$com_code),'id','ASC',51);
if (!empty($data)) {
...
$info->CounterUsed= get_count_where(new Admin(), array("com_code" => $com_code, "permission_roles_id" => $info->id));
...
{
    if(auth()->user()->is_master_admin==0){
        check_permission_sub_menue_actions_redirect(150);
        }   
...
{
    if(auth()->user()->is_master_admin==0){
        check_permission_sub_menue_actions_redirect(151);
        } 
...
public function details($id){
    if(auth()->user()->is_master_admin==0){
        check_permission_sub_menue_actions_redirect(153);
        } 
try{
$com_code=auth()->user()->com_code;
...
$permission_roles_main_menus=get_cols_where(new Permission_roles_main_menus(),array("*"),array("com_code"=>$com_code,"permission_roles_id"=>$id),'id','ASC');
...
if(!empty($info->permission_roles_sub_menu)){
    foreach($info->permission_roles_sub_menu as $sub){
        ...
        $sub->roles_actions=get_cols_where(new roles_actions(),array("*"),array("permission_roles_sub_menu_id"=>$sub->id));
        if(!empty($sub->roles_actions)){
            foreach($sub->roles_actions as $action){
...
{
    if(auth()->user()->is_master_admin==0){
        check_permission_sub_menue_actions_redirect(154);
        } 
try {
$com_code = auth()->user()->com_code;
$data = Permission_rols::select()->find($id);
...
try {
$item_row = roles_actions::find($id);
if (!empty($item_row)) {
$flag = $item_row->delete();
if ($flag) {
return redirect()->back()
...
public function load_add_roles_actions(Request $request){
    if($request->ajax()){
    $com_code = auth()->user()->com_code;        
    ...
    return view('admin.permission_roles.load_add_roles_actions', ['permission_roles_sub_menu' => $permission_roles_sub_menu,'permission_sub_menues_actions'=>$permission_sub_menues_actions]);
    }
    }
...
public function add_roles_actions($permission_roles_sub_menu_id ,Request $request)
{
    if(auth()->user()->is_master_admin==0){
        check_permission_sub_menue_actions_redirect(154);
        } 
...
$dataToInsert['permission_roles_id']=$data['permission_roles_id'];
$checkExists = get_cols_where_row(new roles_actions(),array("id"),$dataToInsert);
if(empty($checkExists)){
$dataToInsert['added_by'] = auth()->user()->id;
$dataToInsert['created_at'] = date("Y-m-d H:i:s");
insert(new roles_actions(),$dataToInsert);
}
}
return redirect()->back()->with(['success' => 'لقد تم اضافة البيانات بنجاح']);
} catch (\Exception $ex) {
...
{
    if(auth()->user()->is_master_admin==0){
        check_permission_sub_menue_actions_redirect(152);
        }
try {
    $com_code = auth()->user()->com_code;
$data = get_cols_where_row(new Permission_rols(),array("id"),array("com_code"=>$com_code,'id'=>$id));
if (!empty($data)) {
 $CounterUsed=get_count_where(new Admin(),array("com_code"=>$com_code,"permission_roles_id"=>$id));
if($CounterUsed>0){
    return redirect()->back()
    ->with(['error' => 'عفوا هذا الدور مستخدم من قبل ومتاح فقط التعطيل والتفعيل عليها']); 
}


$flag = destroy(new Permission_rols(),array("com_code"=>$com_code,'id'=>$id));
if ($flag) {
return redirect()->back()
->with(['success' => '   تم حذف البيانات بنجاح']);
} else {
