...
class ReligionController extends Controller
{
    public function index()
    {
      ...
      foreach($data as $info){
        $info->counterUsed=get_count_where(new Employee(),array("com_code"=>$com_code,"religion_id"=>$info->id) );
     }
    }
    ...
    public function destroy($id)
    {
        try {
            ...
            $counterUsed=get_count_where(new Employee(),array("com_code"=>$com_code,"religion_id"=>$id) );
            if ($counterUsed>0) {
            return redirect()->route('Religions.index')->with(['error' => 'عفوا لايمكن الحذف لانه مستخدم  من قبل فى النظام  ']);
            }
            DB::beginTransaction();
