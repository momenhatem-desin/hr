    ...
    foreach ($rows as $row) {
      ...
      if (!empty($employeeData)) {
        ...
        if (empty($checkExsistsBefor)) {
          ...
          $cheakfor_empty_record = get_cols_where_row(new Attenance_departure(), array("id"), array("employees_code" => $employeeData['employees_code'], "finance_cln_periods_id" => $this->finance_cln_periods_data['id'], 'the_day_date' => $date, 'datetime_In' => null, 'com_code' => $com_code));
          if (!empty($cheakfor_empty_record)) {
            //  لو البصمه الحاليه حضور يبقى نحذف البصمه السابقه
            if ($action_type == 1) {
              destroy(new Attenance_departure(), array("id" => $cheakfor_empty_record['id']));
            }
          }
          ...
          if (!empty($last)) {
            ...
            if ($last['dateIn'] == $date) {
              ...
              if ($minutesiff > $dataSetting['less_than_miniute_neglecting_passmaa']) {
                ...
                $timeenter = $time;
             if ($employeeData['is_has_fixced_shift'] == 1) {
                if ($shiftdata['to_time'] > $timeenter) {
                  $secondsNow = strtotime($shiftdata['to_time']) - strtotime($timeenter);
            ...
            $dataToInsert_departure['branch_id'] = $employeeData['branch_id'];
